...
class MovieBoxProvider : MainAPI() {
    ...
    private fun buildUrl(path: String, params: Map<String, String> = emptyMap()): String {
        ...
        uri.appendQueryParameter("host", mainUrl)
    ...
    private suspend fun fetchLiveSports(): List<SearchResponse> {
        ...
        try {
            val url = buildUrl("/wefeed-mobile-bff/live/sub-upcomming-match")
            val xClientToken = generateXClientToken()
            val xTrSignature = generateXTrSignature("GET", "application/json", "application/json", url)
            ...
            if (response.code == 200) {
                ...
                if (data != null && data.isArray) {
                    ...
                    data.forEach { match ->
                        val subjectId = match["subjectId"]?.asText() ?: match["id"]?.asText()
                        val title = match["title"]?.asText() ?: match["matchName"]?.asText()
                        val coverUrl = match["cover"]?.get("url")?.asText()
                            ?: match["poster"]?.asText()
                        if (subjectId != null && title != null) {
                            liveItems.add(
                                newMovieSearchResponse(title, subjectId, TvType.Live) {
                                    this.posterUrl = coverUrl
                                }
                            )
                        }
        ...
        // Fallback: tab-operating with tabId = 2
        return try {
            val url = buildUrl("/wefeed-mobile-bff/tab-operating", mapOf(
                "page" to "1",
                "tabId" to "2",
            ...
            val mapper = jacksonObjectMapper()
            val root = mapper.readTree(body)
            val sections = root["data"]?.get("items") ?: return emptyList()
            ...
            sections.forEach { section ->
                ...
                val media = when (type) {
                    "SUBJECTS_MOVIE" -> section["subjects"]
                    "CUSTOM" -> section["customData"]?.get("items")?.mapNotNull { it["subject"] }
                    else -> null
                }
                media?.forEach { subject ->
                    val subjectId = subject["subjectId"]?.asText() ?: return@forEach
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val finalUrl = buildUrl("/wefeed-mobile-bff/subject-api/get", mapOf("subjectId" to id))
        val xClientToken = generateXClientToken()
        ...
        var description = data["description"]?.asText()
        ...
        if (type == TvType.Live) {
            var matchDetails: JsonNode? = null
            try {
                val detailUrl = buildUrl("/wefeed-mobile-bff/live/match-detail", mapOf("id" to id))
                val detailSig = generateXTrSignature("GET", "application/json", "application/json", detailUrl)
                val detailHeaders = headers.toMutableMap().apply {
                    put("x-tr-signature", detailSig)
                }
                val detailResponse = app.get(detailUrl, headers = detailHeaders)
                if (detailResponse.code == 200) {
                    val detailBody = detailResponse.body.string()
                    val detailRoot = mapper.readTree(detailBody)
                    matchDetails = detailRoot["data"]
                }
            } catch (_: Exception) { /* ignore */ }

            val liveDescription = if (matchDetails != null) {
                val teamA = matchDetails["teamA"]?.asText()
                val teamB = matchDetails["teamB"]?.asText()
                val scoreA = matchDetails["scoreA"]?.asText()
                val scoreB = matchDetails["scoreB"]?.asText()
                val time = matchDetails["startTime"]?.asText() ?: matchDetails["time"]?.asText()
                val tournament = matchDetails["tournament"]?.asText()
                val status = matchDetails["status"]?.asText()

                buildString {
                    if (teamA != null && teamB != null) {
                        append("$teamA vs $teamB")
                        if (scoreA != null && scoreB != null) {
                            append("  [$scoreA - $scoreB]")
                        }
                        append("\n")
                    }
                    if (tournament != null) append("🏆 $tournament\n")
                    if (time != null) append("⏱️ $time\n")
                    if (status != null) append("📡 $status")
                    else if (description != null) append(description)
                }.takeIf { it.isNotBlank() } ?: (description ?: "Live channel")
            } else {
                description ?: "Live channel"
            }

            val episode = newEpisode("$id|0|0") {
                this.name = "Live Stream"
                this.season = 0
                this.episode = 0
                this.posterUrl = coverUrl
                this.description = liveDescription
            }
            return newTvSeriesLoadResponse(title, finalUrl, type, listOf(episode)) {
                this.posterUrl = coverUrl ?: Poster
                this.backgroundPosterUrl = Background ?: backgroundUrl ?: Poster
                try { this.logoUrl = logoUrl } catch(_: Throwable) {}
                this.plot = liveDescription
                this.year = year
                this.tags = tags
                this.actors = actors
        ...
        if (type == TvType.TvSeries) {
            ...
            for (subjectId in allSubjectIds) {
                val seasonUrl = buildUrl("/wefeed-mobile-bff/subject-api/season-info", mapOf("subjectId" to subjectId))
    ...
    override suspend fun loadLinks(
        ...
        try {
            ...
            if (season == 0 && episode == 0) {
                val liveUrl = buildUrl("/wefeed-mobile-bff/subject-api/play-info", mapOf("subjectId" to originalSubjectId))
                val xClientToken = generateXClientToken()
                val xTrSignature = generateXTrSignature("GET", "application/json", "application/json", liveUrl)
                ...
                val response = app.get(liveUrl, headers = headers)
                if (response.code == 200) {
                    val responseBody = response.body.string()
                    val mapper = jacksonObjectMapper()
                    val root = mapper.readTree(responseBody)
            ...
            // ---- MOVIES & SERIES ----
            val subjectUrl = buildUrl("/wefeed-mobile-bff/subject-api/get", mapOf("subjectId" to originalSubjectId))
            ...
            for ((subjectId, language) in subjectIds) {
                try {
                    val url = buildUrl("/wefeed-mobile-bff/subject-api/play-info", mapOf(
                    ...
                    if (response.code == 200) {
                        ...
                        if (streams != null && streams.isArray) {
                            for (stream in streams) {
                                ...
                                // Subtitles
                                val subLink = buildUrl("/wefeed-mobile-bff/subject-api/get-stream-captions", mapOf("subjectId" to subjectId, "streamId" to id))
                                val xClientToken = generateXClientToken()
                                ...
                                val subLink1 = buildUrl("/wefeed-mobile-bff/subject-api/get-ext-captions", mapOf("subjectId" to subjectId, "resourceId" to id, "episode" to "0"))
                                val xClientToken1 = generateXClientToken()
                                val xTrSignature1 = generateXTrSignature("GET", "", "", subLink1)
                                val subHeaders1 = mapOf(
                                    "Authorization" to "Bearer $token",
                        ...
                        if (streams == null || !streams.isArray || streams.size() == 0) {
                            val fallbackUrl = buildUrl("/wefeed-mobile-bff/subject-api/get", mapOf("subjectId" to subjectId))

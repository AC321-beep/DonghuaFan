    ...
    override suspend fun getUrl(url: String, referer: String?, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit) {
        ...
        if (matches.isNotEmpty()) {
            ...
            if (jsonMatch != null) {
                ...
                } catch (e: Exception) { Log.e("AbyssPlayer", "Failed to parse inner config base64") }
            }
        }
    }
}

class Embedwish : StreamWishExtractor() { override var name = "Embedwish"; override var mainUrl = "https://embedwish.com" }
class Filelions : StreamWishExtractor() { override var name = "Filelions"; override var mainUrl = "https://filelions.live" }
class Swhoi : StreamWishExtractor() { override var name = "Swhoi"; override var mainUrl = "https://swhoi.com"; override val requiresReferer = true }
class VidHidePro5 : VidHidePro() { override var name = "VidHidePro"; override val mainUrl = "https://vidhidevip.com"; override val requiresReferer = true }
...
class P2pstream : ExtractorApi() {
    ...
    override val requiresReferer = true
    override suspend fun getUrl(url: String, referer: String?, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit) {
        manualJsUnpackExtraction(url.replace("/#", "/e/"), name, mapOf("Origin" to mainUrl, "Referer" to "$mainUrl/"), callback)
    }
}

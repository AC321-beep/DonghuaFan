...
class AnimekhorProvider : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        coroutineScope {
            document.select(".mobius option, select.mirror option, .server-list li a[data-embed], .server-list li a[data-em]").map { server ->
                async {
                ...
                ajaxElements.map { element ->
                    async {
                        ...
                        if (postId.isNotBlank() && nume.isNotBlank()) {
                            ...
                            val iframeSrc = Jsoup.parse(cleanResponse).selectFirst("iframe")?.attr("src") ?: Regex("""src=["'](.*?)["']""").find(cleanResponse)?.groupValues?.get(1) ?: Regex("""(https?://[^"'\s]+)""").find(cleanResponse)?.groupValues?.get(1)
        ...
        document.select("#embed_holder iframe, .playerx iframe, .video-content iframe").forEach { iframe ->
            ...
            if (src.isNotBlank() && !src.contains("youtube", true) && !src.contains("disqus", true)) invokeExtractor(src, "Direct Server")
        }

...
class DonghuastreamProvider : MainAPI() {
    ...
    companion object {
        private const val TAG = "DonghuaStream"
    ...
    override suspend fun loadLinks(
        ...
        val doc = try { app.get(data, headers = headers).document } catch (e: Exception) { null } ?: return false
        ...
        doc.select("iframe[src*='dailymotion']").forEach { iframe ->
            ...
            if (match != null) dailymotionToken = match.groupValues[1]
        }
        ...
        // 2) Fallback: player_aaaa Dailymotion
        ...
        // 3) All other iframes -> let extractor system handle them
